-- 教育网申领：每轮人数上限 + 满额自动关窗
-- 在 Supabase SQL Editor 或 apply_migration 执行（需在 20260330_edu_credit_grant 之后）。

-- ─────────────────────────────────────────
-- 1. 列 max_claims
-- ─────────────────────────────────────────
ALTER TABLE public.edu_credit_grant_windows
...
ALTER TABLE public.edu_credit_grant_windows
  ALTER COLUMN max_claims SET DEFAULT 10,
  ALTER COLUMN max_claims SET NOT NULL;

ALTER TABLE public.edu_credit_grant_windows
  DROP CONSTRAINT IF EXISTS edu_credit_grant_windows_max_claims_check;

ALTER TABLE public.edu_credit_grant_windows
  ADD CONSTRAINT edu_credit_grant_windows_max_claims_check CHECK (max_claims >= 1);

-- ─────────────────────────────────────────
-- 2. 替换 open：双参（先删旧单参重载）
-- ─────────────────────────────────────────
DROP FUNCTION IF EXISTS public.open_edu_credit_grant_window(uuid);

CREATE OR REPLACE FUNCTION public.open_edu_credit_grant_window(
  p_admin_id     uuid,
  p_max_claims   integer DEFAULT 10
)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  IF p_max_claims IS NULL OR p_max_claims < 1 THEN
    RAISE EXCEPTION 'EDU_GRANT_INVALID_MAX_CLAIMS';
  END IF;
  ...
  INSERT INTO public.edu_credit_grant_windows (opened_by, max_claims)
  VALUES (p_admin_id, p_max_claims)
...
REVOKE ALL ON FUNCTION public.open_edu_credit_grant_window(uuid, integer) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.open_edu_credit_grant_window(uuid, integer) TO service_role;

-- ─────────────────────────────────────────
-- 3. 替换 claim：名额校验 + 满额关窗
-- ─────────────────────────────────────────
